...
from calmodel.evaluation.v2_m1 import (
...
from calmodel.model.body_hypotheses import BodyGraphHypothesisFilter
from calmodel.model.entity_graph import OnlineEntityGraph
from calmodel.model.online_control import (
    ControlAgentConfig,
    OnlineControlAgent,
)
...
def _verify_locked_sources(protocol: dict[str, Any]) -> dict[str, str]:
    observed = {
        ...
        for path in protocol["locked_source_sha256"]
    }
    mismatches = {
        path: {
            "expected": protocol["locked_source_sha256"][path],
            "observed": digest,
        }
        for path, digest in observed.items()
        if digest != protocol["locked_source_sha256"][path]
    }
    if mismatches:
        raise RuntimeError(
