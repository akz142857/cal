    ...
    *,
    ...
    chain = {
        ...
        "all_formal_stage_gates_pass": all(
            ...
            for item in (m1, m2, m3, confirmation, m4)
        ),
    }
    ...
    summary = {
        ...
        "key_results": {
            ...
            "m4_occupancy_iou": m4["aggregate"]["occupancy_iou"],
            "m4_hidden_recall": m4["aggregate"]["hidden_occupancy_recall"],
            "m4_active_step_reduction": m4["aggregate"][
    ...
    report.write_text(
        _render_report(summary, m1, m2, m3, confirmation, m4),
        encoding="utf-8",
    )
    return summary
...
| V2-M4 | 特权可见性诊断：占据 IoU {key['m4_occupancy_iou']:.3f}；遮挡召回 {key['m4_hidden_recall']:.3f} | {m4['decision']} |
...
{m4['aggregate']['moving_hidden_probability']:.3f}，但仍读取模拟器可见性掩码。
