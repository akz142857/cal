"""Machine-readable pre-registered decision for M1e part slots."""
...
def build_m1e_screen_summary(
    root: str | Path = "results/M1e-part-slots-screen",
    output: str | Path = "results/M1e-screen-summary.json",
) -> dict[str, Any]:
    source = Path(root)
    records = {}
    for condition in ("gru", "no_action", "feedforward", "no_persistence"):
        run = source / f"m1e_part_slots_{condition}" / "seed-000"
    ...
    summary = {
        ...
        "candidate": "M1e-persistent-controllable-part-slots",
        "model_seeds": [0],
        "records": records,
        "gates": gates,
        "temporal_gate": {
        ...
        "decision": (
            "run_temporal_screen"
            if passed
            else "stop_M1e_before_temporal_and_five_seed_confirmation"
        ),
        "preregistration": "docs/experiments/M1E_PREREGISTRATION.md",
...
if __name__ == "__main__":
    result = build_m1e_screen_summary()
