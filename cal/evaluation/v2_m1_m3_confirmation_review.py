...
def build_v2_m1_m3_confirmation_review(
    *,
    ...
    v5_protocol_path: str | Path = (
        "experiments/V2_M1_M3_INTEGRATED_CONFIRMATION_PROTOCOL_V5.json"
    ),
    ...
    gates = {
        ...
        "v5_amendment_preserves_v4_protocol": (
            v5_protocol["amendment_record"]["prior_protocol_sha256"]
            == digests["v4_protocol"]
        ),
        "v5_amendment_preserves_confirmation": (
            v5_protocol["amendment_record"][
                "prior_confirmation_result_sha256"
            ]
            == digests["v2_confirmation"]
        ),
        "v5_extension_kept_thresholds_and_verified_equivalence": (
            v5_protocol["fixed_gates"] == v4_protocol["fixed_gates"]
            and v5_protocol["amendment_record"][
                "model_or_stage_algorithm_changed"
            ]
            is False
            and v5_protocol["amendment_record"][
        ...
        "v6_amendment_preserves_v5_protocol": (
            v6_protocol["amendment_record"]["prior_protocol_sha256"]
            == digests["v5_protocol"]
        ),
        "v6_amendment_preserves_confirmation": (
            v6_protocol["amendment_record"][
                "prior_confirmation_result_sha256"
            ]
            == digests["v2_confirmation"]
        ),
        "v6_extension_kept_thresholds_and_verified_equivalence": (
            v6_protocol["fixed_gates"] == v5_protocol["fixed_gates"]
            and v6_protocol["amendment_record"][
                "model_or_stage_algorithm_changed"
            ]
            is False
            and v6_protocol["amendment_record"][
...
随后发布的 v5 修订（SHA-256 `{summary['protocol_v5_sha256']}`）给
...
随后发布的 v6 修订（SHA-256 `{summary['protocol_v6_sha256']}`）给
`OnlineEntityGraph` 加了一个默认值为 `None`（无操作）的
`drift_reset_after` 构造参数；M1/M2/M3/本确认脚本均未传入该参数，
行为不变。这次是为 V2-I1 系统集成探针诊断出的问题设计的，但实测
对该探针的目标指标（`self_f1`/`identity_consistency`）没有超出
噪声量级的真实影响，因此**没有**被接入 V2-I1 的默认配置——截至
本报告生成时，仓库里没有任何调用方主动启用这个参数，它是一个
已验证、已测试、但当前未被使用的能力（详见
docs/experiments/V2_I1_INTEGRATION_REPORT.md 的 V6 节）。同样是
逐字段重跑验证而非论证：v6 修改前后的 M2（三种关联模式）、M3
